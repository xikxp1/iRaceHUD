<script lang="ts">
    ...
    let settings = $state<TelemetryOverlaySettings | undefined>(undefined);
    ...
    onMount(() => {
        invoke<TelemetryOverlaySettings>("get_telemetry_overlay_settings").then(
            (x) => {
                settings = x;
            },
        );
    ...
    function handleEnabledChange(event: Event) {
        ...
        invoke("set_telemetry_overlay_settings", { settings: settings });
    }

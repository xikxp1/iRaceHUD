<script lang="ts">
    import type { TrackMapWidgetSettings } from "$lib/types/telemetry";
    import { invoke } from "@tauri-apps/api/core";
    import { onMount } from "svelte";

    let settings = $state<TrackMapWidgetSettings | undefined>(undefined);
    ...
    onMount(() => {
        invoke<TrackMapWidgetSettings>("get_track_map_widget_settings").then(
            (x) => {
                settings = x;
            },
        );
    ...
    async function handleEnabledChange(event: Event) {
        ...
        invoke("set_track_map_widget_settings", { settings: settings });
    }
